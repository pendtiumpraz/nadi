...
export async function getEventsByAuthor(authorId: string): Promise<NADIEvent[]> {
    const sql = getDB();
    const rows = await sql`SELECT * FROM events WHERE author_id = ${Number(authorId)} ORDER BY date DESC`;
    return rows.map(rowToEvent);
}

export async function saveEvent(event: NADIEvent): Promise<void> {
    const sql = getDB();
    const publishStatus: EventPublishStatus = event.publishStatus || "published";
    const authorId = event.authorId ? Number(event.authorId) : null;
    const feedbackPending = !!event.feedbackPending;
    await sql`
    INSERT INTO events (slug, title, description, date, time, location, location_type, category, image_url, registration_url, status, speakers, organizer, publish_status, author_id, feedback_pending)
    VALUES (${event.slug}, ${event.title}, ${event.description}, ${event.date}, ${event.time}, ${event.location}, ${event.locationType}, ${event.category}, ${event.imageUrl}, ${event.registrationUrl || ""}, ${event.status}, ${event.speakers || []}, ${event.organizer}, ${publishStatus}, ${authorId}, ${feedbackPending})
...
export async function updateEventPublishStatus(slug: string, status: EventPublishStatus): Promise<void> {
    const sql = getDB();
    await sql`UPDATE events SET publish_status = ${status} WHERE slug = ${slug}`;
}

export async function setEventFeedbackPending(slug: string): Promise<void> {
    const sql = getDB();
    await sql`UPDATE events SET feedback_pending = true WHERE slug = ${slug}`;
}

export async function clearEventFeedbackPending(slug: string): Promise<void> {
    const sql = getDB();
    await sql`UPDATE events SET feedback_pending = false WHERE slug = ${slug}`;
}
...
function rowToEvent(row: Record<string, unknown>): NADIEvent {
    return {
        ...
        publishStatus: (row.publish_status as EventPublishStatus) || "published",
